...
BPRSS <- BPRSL %>%
  ...
  summarise(mean = mean(bprs, na.rm = TRUE), 
            se   = (sd(bprs)/sqrt(n))) %>%
  ungroup()
...
library(ggplot2)
ggplot(BPRSS, aes(x = week, y = mean, linetype = treatment, shape = treatment, colour = treatment)) +
  geom_line() +
...
anova(RATS_ref1, RATS_ref, RATS_reg)
